...
from pyimagesearch.covermatchercache import CoverMatcher
import argparse
import glob
import os
import csv
...
rns=input('Enter Photo#[pNN] or IMG_#[INNNN]:')
if not rns:
        sys.exit(0)
ns=rns[1:]
if rns[0].lower()=='i':
        queryn='IMG_%s.JPG'%(ns,)
if rns[0].lower()=='p':
        queryn='photo%s.JPG'%(ns,)
#queryn='photo32.JPG'  #  case sens
print('going to search for match to {}'.format(queryn))
...
#minMatches = 40
minMatches = 10
print('minMatches={}'.format(minMatches))
...
queryfp=os.path.join(queryfd,queryn)  #  queryn=img8250.jpg

queryImage = cv2.imread(queryfp)
gray = cv2.cvtColor(queryImage, cv2.COLOR_BGR2GRAY)
print('starting query describe at {}'.format(ct()))
(queryKps, queryDescs) = cd.describe(gray)
print('finish query describe at {}'.format(ct()))
...
print('query image={}'.format(queryn))
